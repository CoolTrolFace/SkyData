...
import com.emre.skydata.DataMain;
...
public class YamlItem {
    ...
    public YamlItem(final String path) {
        this.path = path;
        this.setType(((DataMain.config.get(path + ".type") instanceof String)) ? Material.valueOf(DataMain.config.getString(path + ".type").toUpperCase()) : Material.AIR);
        this.setName(((DataMain.config.get(path + ".name") instanceof String)) ? DataMain.config.getString(path + ".name") : null);
        this.setLore(DataMain.config.getStringList(path + ".lore"));
        this.setSlot(((DataMain.config.get(path + ".slot") instanceof Integer)) ? DataMain.config.getInt(path + ".slot") : 0);
        this.setAmount(((DataMain.config.get(path + ".amount") instanceof Integer)) ? DataMain.config.getInt(path + ".amount") : 1);
        this.setGlow((DataMain.config.get(path + ".glow") instanceof Boolean) ? DataMain.config.getBoolean(path + ".glow") : false);
        this.setHeadValue(((DataMain.config.get(path + ".head-value") instanceof String)) ? DataMain.config.getString(path + ".head-value") : null);
    }
    ...
    public String getKey(String key) {
        return DataMain.config.getString(this.path + "."+key);
    }
    ...
    public YamlItem renewLore() {
        this.setLore(DataMain.config.getStringList(this.path + ".lore"));
        return this;
    }
    ...
    public ItemStack complete(){
        ...
        if(this.headValue == null){
            ...
            if(((DataMain.config.get(path + ".model"))) instanceof Integer){
                net.minecraft.server.v1_16_R3.ItemStack nmsitem = CraftItemStack.asNMSCopy(item);
                NBTTagCompound nbtset = new NBTTagCompound();
                if (nmsitem.hasTag()) {
                    nbtset.a(nmsitem.getTag());
                }
                nbtset.setInt("CustomModelData", DataMain.config.getInt(path + ".model"));
